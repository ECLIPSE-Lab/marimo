...
def _(
    ...
    vertical2 = mo.vstack(
        [horizontal, fig_pctf, horizontal2],
        align="start",
        gap=0
    )
...
def _(np, probe_real, sh):
    ...
    # so we keep the real part. In focus it is smooth, positive and monotonic.
    probe_intensity = np.abs(probe_real) ** 2
    intensity_fft = np.real(
        np.fft.fftshift(np.fft.fft2(np.fft.ifftshift(probe_intensity)))
    )
    cy, cx = sh[0] // 2, sh[1] // 2
    yy, xx = np.indices(intensity_fft.shape)
