...
def _():
    ...
    return cmr, ctf, np, plt


@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""## 4D STEM Simulation""")
    return
...
def _(cmr):
    ...
    return (
        ...
        k_probe,
        phi0,
        pixelated_parallax_line_color,
        pixelated_ssb_line_color,
        reciprocal_sampling,
        sampling,
        scan_step_size,
        segmented_parallax_line_color,
        segmented_ssb_line_color,
        wavelength,
    )
...
@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""#### Import sample potentials""")
    return
...
def _(
    ...
    return (
        ...
        kx,
        ky,
        return_complex_probe,
        return_simulation_inputs,
        simulation_inputs,
        soft_aperture,
    )
...
@app.cell
def _(complex_obj, ctf, gpts, np, scan_step_size, simulation_inputs):
    ...
    return intensities, simulate_intensities
...
def _(
    ...
    def return_reconstruction_ctfs(
        ...
        return ctf_arrays
    return (
        return_aperture_overlap_factor,
        return_full_gamma_factor,
        return_reconstruction_ctfs,
    )
...
def _(
    ...
    return (return_aperture_factors,)
...
def _(gpts):
    ...
    defocus_slider = mo.ui.slider(start=-gpts, stop=gpts, label="negative defocus, $C_{1,0}$ [Å]", value=gpts)
 
    subsampling_slider = mo.ui.dropdown(
        options=[2,3,4,6],
        value=4, label="sub-sampling"
    )

    object_dropdown = mo.ui.dropdown(
        options={"strontium titanate": 0, "metal-organic framework": 1, "apoferritin protein": 1} ,
        value="apoferritin protein",
    )

    simulate_button = mo.ui.button(
        value=0, on_click=lambda value: value + 1, label="simulate (expensive)"
    )
 
    reconstruct_button = mo.ui.button(
        label='reconstruct (expensive)', 
    )
 

    return defocus_slider, mo, reconstruct_button, subsampling_slider
...
def _(
    ...
    ctf.complex_to_rgb(np.fft.fftshift(arr))
